...
if(isset($_GET['id'])){
    ...
    if($qry->num_rows >0){
        ...
        if($from_order == 1){
            $qry = $conn->query("SELECT p.*,s.name as warehouse FROM stock_in_list p inner join warehouse_list s on p.warehouse_id = s.id  where p.id = '{$form_id}'");
            if($qry->num_rows >0){
                foreach($qry->fetch_array() as $k => $v){
                    if($k == 'id')
                    $k = 'po_id';
            ...
            $qry = $conn->query("SELECT b.*,s.name as warehouse,p.po_code FROM back_order_list b inner join warehouse_list s on b.warehouse_id = s.id inner join stock_in_list p on b.po_id = p.id  where b.id = '{$_GET['bo_id']}'");
            if($qry->num_rows >0){
                foreach($qry->fetch_array() as $k => $v){
                    if($k == 'id')
                    $k = 'bo_id';
...
if(isset($_GET['po_id'])){
    $qry = $conn->query("SELECT p.*,s.name as warehouse FROM stock_in_list p inner join warehouse_list s on p.warehouse_id = s.id  where p.id = '{$_GET['po_id']}'");
    if($qry->num_rows >0){
        foreach($qry->fetch_array() as $k => $v){
            if($k == 'id')
            $k = 'po_id';
            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select id="warehouse_id" name="warehouse_id" class="custom-select select2">
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tbody>
                        ...
                        if(isset($po_id)):
                        if(!isset($bo_id))
                        $qry = $conn->query("SELECT p.*,i.name,i.classification FROM `po_items` p inner join item_list i on p.item_id = i.id where p.po_id = '{$po_id}'");
                        else
                        $qry = $conn->query("SELECT b.*,i.name,i.classification FROM `bo_items` b inner join item_list i on b.item_id = i.id where b.bo_id = '{$bo_id}'");
                        ...
                        <tr>
                            ...
                            <td class="py-1 px-2 text-center qty">
                                ...
                                <input type="hidden" name="packaging_size[]" value="<?php echo $row['packaging_size']; ?>">
...
<script>
    ...
    function rem(_this){
        ...
        if($('table#list tbody tr').length <= 0)
            $('#warehouse_id').removeAttr('readonly')

...
if($qry->num_rows >0){
    ...
    if($from_order == 1){
        $po_qry = $conn->query("SELECT p.*,s.name as warehouse FROM `stock_in_list` p inner join `warehouse_list` s on p.warehouse_id = s.id where p.id= '{$form_id}' ");
        ...
        $qry = $conn->query("SELECT b.*,s.name as warehouse,p.po_code FROM back_order_list b inner join warehouse_list s on b.warehouse_id = s.id inner join stock_in_list p on b.po_id = p.id  where b.id = '{$form_id}'");
            if($qry->num_rows >0){
                foreach($qry->fetch_array() as $k => $v){
                    if($k == 'id')
                    $k = 'bo_id';
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div><?php echo isset($warehouse) ? $warehouse : '' ?></div>
                    </div>
                </div>
                <?php if(isset($bo_id)): ?>
                <div class="col-md-6">
            ...
            <table class="table table-striped table-bordered" id="list">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="py-1 px-2 text-center"><?php echo ($row['packaging_size']) ?></td>
                        <td class="py-1 px-2">

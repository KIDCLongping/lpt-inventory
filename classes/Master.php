...
Class Master extends DBConnection {
	...
	function delete_po(){
		...
		$del = $this->conn->query("DELETE FROM `stock_in_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
	...
	function save_receiving(){
		...
		if($save){
			...
			if(isset($bo_ids)){
				$this->conn->query("UPDATE `stock_in_list` set status = 1 where id = '{$po_id}'");
				if($from_order == 2){
				...
				$this->conn->query("UPDATE `stock_in_list` set status = 2 where id = '{$po_id}'");
				if($from_order == 2){
	...
	function delete_receiving(){
		...
		if($del){
			...
			if(isset($res)){
				if($res['from_order'] == 1){
					$this->conn->query("UPDATE stock_in_list set status = 0 where id = '{$res['form_id']}' ");
				}
			}
		}else{
			$resp['status'] = 'failed';
	...
	function delete_bo(){
		...
		if($del){
			...
			if(isset($bo_res)){
				...
				if($check->num_rows > 0){
					$this->conn->query("UPDATE `stock_in_list` set status = 1 where id = '{$bo_res['po_id']}' ");
				}else{
					$this->conn->query("UPDATE `stock_in_list` set status = 0 where id = '{$bo_res['po_id']}' ");
				}
			}
		}else{
			$resp['status'] = 'failed';
	...
	function save_return(){
		...
		if($save){
			...
			foreach($item_id as $k =>$v){
				$sql = "INSERT INTO `stock_list` set item_id='{$v}', `quantity` = '{$qty[$k]}', `packaging_size` = '{$packaging_size[$k]}', `price` = '{$price[$k]}', `total` = '{$total[$k]}', `type` = 2 ";
				$save = $this->conn->query($sql);
				if($save){
